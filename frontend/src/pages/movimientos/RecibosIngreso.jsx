...
const createEmptyForm = () => ({
  nu_codi_recibo: '',
  dt_fech_ingr: toDateTimeInputValue(),
  ch_seri_ingr: '',
  ch_nume_ingr: '',
  ch_esta_activo: '1',
...
export default function RecibosIngreso() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [rRes, tRes, cRes, cajerosRes] = await Promise.all([
      ...
      setTiposIngreso(normalizeList(tRes.data));
  ...
  const rows = useMemo(
    () =>
      data.map((item) => ({
        ...item,
        tipo_desc: item.tipo_ingreso_desc || '-',
  ...
  const totalIngresos = useMemo(
    () => rows.reduce((sum, row) => sum + Number(row.nu_impo_ingr || 0), 0),
    [rows],
  );
  ...
  const handleSelect = (row) => {
    ...
    setForm({
      ...
      dt_fech_ingr: toDateTimeInputValue(row.dt_fech_ingr),
      ch_seri_ingr: row.ch_seri_ingr ?? '',
      ch_nume_ingr: row.ch_nume_ingr ?? '',
      ch_esta_activo: row.ch_esta_activo ?? '1',
      ch_codi_tipo_ingreso: row.ch_codi_tipo_ingreso ?? '',
      ch_codi_cliente: row.ch_codi_cliente ?? '',
      nu_impo_ingr: row.nu_impo_ingr ?? '',
      vc_obse_ingr: row.vc_obse_ingr ?? '',
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...form,
        ch_codi_cliente: form.ch_codi_cliente || null,
      };
      delete payload.nu_codi_recibo;

      if (editId) {
        await api.put(`/movimientos/recibos-ingreso/${editId}/`, payload);
      } else {
        await api.post('/movimientos/recibos-ingreso/', payload);
      }
  ...
  const handleDelete = async () => {
    ...
    await api.patch(`/movimientos/recibos-ingreso/${editId}/`, { ch_esta_activo: '0' });
    await fetchData();
    handleNew();
  };
        ...
        <section style={styles.listPanel}>
          <div style={styles.panelHeader}>
            <div>
              ...
              <h2 style={styles.title}>Listado de ingreso</h2>
              <p style={styles.subtitle}>Consulta, filtra visualmente y selecciona recibos registrados.</p>
            </div>
          ...
          <div style={styles.kpiRow}>
            ...
            <MetricCard label="Total acumulado" value={formatMoney(totalIngresos)} highlight />
          </div>
                ...
                {!loading &&
                  rows.map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        <td style={styles.td}>{formatDate(row.dt_fech_ingr)}</td>
                        <td style={styles.td}>{row.ch_seri_ingr || '-'}</td>
                        <td style={styles.td}>{row.ch_nume_ingr || '-'}</td>
                        <td style={styles.td}>{row.tipo_desc}</td>
                        <td style={{ ...styles.td, ...styles.amountCell }}>{formatMoney(row.nu_impo_ingr)}</td>
                        <td style={styles.td}>
        ...
        <section style={styles.formPanel}>
          <div style={styles.formHeader}>
            <div>
              ...
              <h3 style={styles.formTitle}>{editId ? 'Editar ingreso' : 'Nuevo ingreso'}</h3>
              <p style={styles.formSubtitle}>Completa el formulario para registrar o actualizar el recibo.</p>
            </div>
            {editId && (
          ...
          <div style={styles.section}>
            ...
            <div style={styles.formGrid}>
              ...
              <Field label="N° Serie" value={form.ch_seri_ingr} readOnly placeholder="Auto" />
              <Field label="Numero" value={form.ch_nume_ingr} readOnly placeholder="Auto" />
              <Field
                label="Estado"
                type="select"
              ...
              <Field
                label="Fecha"
                type="datetime-local"
                value={form.dt_fech_ingr}
                onChange={(v) => handleChange('dt_fech_ingr', v)}
              />
              <Field
                label="Tipo de ingreso"
                type="select"
                value={form.ch_codi_tipo_ingreso}
                onChange={(v) => handleChange('ch_codi_tipo_ingreso', v)}
                options={[
                  { value: '', label: 'Seleccionar' },
                  ...tiposIngreso.map((tipo) => ({
                    value: tipo.ch_codi_tipo_ingreso,
                    label: tipo.vc_desc_tipo_ingreso,
                  })),
                ]}
              />
            </div>
          ...
          <div style={styles.section}>
            <p style={styles.sectionTitle}>Relacion comercial</p>
            <div style={styles.formGrid}>
              <Field
                label="Cliente"
                type="select"
                value={form.ch_codi_cliente}
                onChange={(v) => handleChange('ch_codi_cliente', v)}
                options={[
                  { value: '', label: 'Seleccionar' },
                  ...clientes.map((cliente) => ({
                    value: cliente.ch_codi_cliente,
                    label: cliente.vc_razo_soci_cliente,
                  })),
                ]}
              />
              <Field
                label="Total"
                type="number"
                value={form.nu_impo_ingr}
                onChange={(v) => handleChange('nu_impo_ingr', v)}
              />
              <Field
                label="Cajero"
                type="select"
              ...
              <Field
                label="Observacion"
                type="textarea"
                value={form.vc_obse_ingr}
                onChange={(v) => handleChange('vc_obse_ingr', v)}
              />
            </div>
          </div>
