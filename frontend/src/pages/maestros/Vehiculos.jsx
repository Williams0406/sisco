...
const COLUMNS = [
  ...
  { key: 'ch_esta_parqueado', label: 'Parqueado',
    render: (v) => v === '1' ? '🚗 Sí' : '—' },
...
export default function Vehiculos() {
  ...
  const getFields = () => [
    ...
    { key: 'ch_codi_cliente',  label: 'Cliente', type: 'select',
      options: clientes.map(c => ({ value: c.ch_codi_cliente, label: c.vc_razo_soci_cliente })) },
    { key: 'ch_codi_chofer',   label: 'Chofer', type: 'select',
      options: choferes.map(c => ({ value: c.ch_codi_chofer, label: c.vc_desc_chofer })) },
  ...
  const handleEdit   = (row) => { setForm({ ...row }); setEditId(row.ch_codi_vehiculo); setModal(true); };
  ...
  const handleSave = async () => {
    ...
    try {
      editId
        ? await api.put(`/maestros/vehiculos/${editId}/`, form)
        : await api.post('/maestros/vehiculos/', form);
  ...
  return (
    <>
      <DataTable title="Vehículos" columns={COLUMNS} data={data}
